...
const unescapeMarkdownUrl = (value) =>
  String(value)
    .replace(/&amp;/g, "&")
    .replace(/&quot;/g, '"')
    .replace(/&#39;/g, "'");
...
export const renderMarkdownInline = (value = "") => {
  ...
  const withCodePlaceholders = String(value).replace(
    /`([^`]+)`/g,
    (_, code) => {
...
const renderTable = (table) => {
  const headers = table.headers
    .map((header) => `<th>${renderMarkdownInline(header)}</th>`)
    .join("");
...
export const renderMarkdownToHtml = (markdown = "") => {
  ...
  while (index < lines.length) {
    ...
    if (headingMatch) {
      ...
      blocks.push(
        `<h${level}>${renderMarkdownInline(headingMatch[2].replace(/\s+#+$/, ""))}</h${level}>`,
      );
      index += 1;
      continue;
    }
    ...
    if (/^>\s?/.test(trimmed)) {
      ...
      blocks.push(
        `<blockquote><p>${renderMarkdownInline(quoteLines.join(" "))}</p></blockquote>`,
      );
      continue;
    }
    ...
    if (/^\|/.test(trimmed)) {
      ...
      blocks.push(
        table
          ? renderTable(table)
          : `<p>${renderMarkdownInline(tableLines.join(" "))}</p>`,
      );
      continue;
    }
    ...
    blocks.push(
      `<p${className}>${renderMarkdownInline(paragraphLines.join(" "))}</p>`,
    );
    paragraphIndex += 1;
  }
