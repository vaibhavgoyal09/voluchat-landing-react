...
test("layout renders the cookie consent component", async () => {
  ...
  assert.match(layout, /import CookieConsent from ['"]\.\.\/components\/CookieConsent\.astro['"]/);
  assert.match(layout, /<CookieConsent \/>/);
});

test("cookie consent persists accept and reject decisions", async () => {
  const component = await readFile(new URL("../src/components/CookieConsent.astro", import.meta.url), "utf8");
...
test("cookie consent accepts analytics when the visitor scrolls", async () => {
  ...
  assert.match(component, /Math\.abs\(window\.scrollY - initialScrollY\) < SCROLL_ACCEPT_THRESHOLD/);
  assert.match(component, /window\.addEventListener\('scroll', handleScrollConsent/);
  assert.match(component, /window\.removeEventListener\('scroll', handleScrollConsent/);
});

test("cookie consent tells visitors that scrolling accepts analytics", async () => {
  const component = await readFile(new URL("../src/components/CookieConsent.astro", import.meta.url), "utf8");

  assert.match(component, /Scrolling this page also accepts analytics cookies\./);
});
