...
export const renderBackgroundSvg = (
  geometry,
...
const probeVideo = (source) => {
  ...
  if (result.status !== 0 || !result.stdout) {
    throw new Error(
      result.stderr || result.error?.message || `Could not probe ${source}.`,
    );
  }
  ...
  if (result.status !== 0) {
    throw new Error(
      result.stderr?.toString() ||
        ...
        `ffmpeg exited with status ${result.status ?? "unknown"}.`,
    );
  }
...
const renderSvgToPng = (source, output) => {
  runFfmpeg(["-y", "-i", source, "-frames:v", "1", "-update", "1", output], {
    encoding: "utf8",
  });
};
...
export const exportHeroPhoneMockup = ({
  source = DEFAULT_SOURCE,
  output = DEFAULT_OUTPUT,
} = {}) => {
...
if (isMain) {
  ...
  if (options.help) {
    ...
    console.log(
      `Exported ${output} (${geometry.canvas.width}x${geometry.canvas.height}).`,
    );
  }
}
